...
func TestIntegrationSkipAll(t *testing.T) {
	err := RunAllTests(context.Background(), config.Config{
		...
		Timeout:   2,
		PingCount: 1,
		Skip:      []string{"ping", "dns", "configured_dns", "records", "mail", "dnssec", "routing", "rdap", "tcp", "udp", "tls", "http", "https", "http3", "cdn"},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunAllTestsLocalHTTP(t *testing.T) {
	...
	err = RunAllTests(context.Background(), config.Config{
		...
		PingCount: 1,
		HTTPPort:  port,
		Skip:      []string{"ping", "dns", "configured_dns", "records", "mail", "dnssec", "routing", "rdap", "udp", "tls", "https", "tcp", "http3", "cdn"},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunAllTestsLocalTCP(t *testing.T) {
	...
	err = RunAllTests(context.Background(), config.Config{
		...
		PingCount: 1,
		TCPPort:   port,
		Skip:      []string{"ping", "dns", "configured_dns", "records", "mail", "dnssec", "routing", "rdap", "udp", "tls", "http", "https", "http3", "cdn"},
	})
	if err != nil {
		t.Fatal(err)
	}
